...
public class NetTCPSSL : NetTCP {
	...
	var ensureSSL: UnsafeMutablePointer<SSL>? {
		if ssl == nil {
			ssl = SSL_new(getCtx(forHost: nil)?.sslCtx)
	...
	public var peerCertificate: X509? {
		guard let ssl = self.ssl, let cert = SSL_get_peer_certificate(ssl) else {
			return nil
		}
		...
		set(list) {
			...
			if let ctx = sslCtx {
				if 0 == SSL_CTX_set_cipher_list(ctx, listStr) {
					print("SSL_CTX_set_cipher_list failed: \(errorStr(forCode: Int32(errorCode())))")
				}
			}
			if let ssl = self.ssl {
	...
	func passwordCallback(_ buf: UnsafeMutablePointer<Int8>, size: Int32, rwflag: Int32) -> Int32 {
		...
		memmove(buf, keyFilePassword, chars.count)
		buf[chars.count] = 0
	...
	fileprivate func makeSSLCTX() -> UnsafeMutablePointer<SSL_CTX>? {
		...
		case .sslV2or3: newSslCtx = SSL_CTX_new(SSLv23_method())
		}
	...
	override public func initSocket(family: Int32) {
		...
		if !keyFilePassword.isEmpty {
			...
			SSL_CTX_set_default_passwd_cb_userdata(sslCtx!, opaqueMe)
			SSL_CTX_set_default_passwd_cb(sslCtx!, callback)
		}
	}
	...
	override func isEAgain(err er: Int) -> Bool {
		if er == -1 && usingSSL {
			let sslErr = SSL_get_error(ssl!, Int32(er))
	...
	override func send(_ buf: [UInt8], offsetBy: Int, count: Int) -> Int {
		...
		if usingSSL {
			let i = Int(SSL_write(ssl!, ptr, Int32(count)))
			return i
		}
	...
	public func beginSSL(closure: @escaping (Bool) -> ()) {
		beginSSL(timeoutSeconds: 5.0, closure: closure)
	}
	...
	private func accepted(_ net: NetTCP?, callBack: @escaping (NetTCP?) -> ()) {
		if let netSSL = net as? NetTCPSSL {
			netSSL.trackCtx = trackCtx
			netSSL.ssl = SSL_new(sslCtx!)
			SSL_set_fd(netSSL.ssl!, netSSL.fd.fd)
			finishAccept(timeoutSeconds: -1, net: netSSL, callBack: callBack)
		} else {
			callBack(net)
		}
	}
	...
	override public func forEachAccept(callBack: @escaping (NetTCP?) -> ()) {
		super.forEachAccept { [unowned self]
			net in
	...
	override public func accept(timeoutSeconds timeout: Double, callBack: @escaping (NetTCP?) -> ()) throws {
		try super.accept(timeoutSeconds: timeout) { [unowned self]
			net in
			self.accepted(net, callBack: callBack)
		}
	}
...
extension NetTCPSSL {
	fileprivate func getCtx(forHost: String?) -> AutoFreeSSLCTX? {
		guard let forHost = forHost else {
			if trackCtx == nil {
				trackCtx = AutoFreeSSLCTX(makeSSLCTX())
			}
			return trackCtx
		}
...
extension NetTCPSSL {
	...
	func enableSNIServer() {
		guard let ctx = sslCtx, !sniContextMap.isEmpty else {
			return
		}
